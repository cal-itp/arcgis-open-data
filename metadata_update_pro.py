...
def overwrite_topic_section(metadata: dict, dataset_info: dict) -> dict:
    ...
    (id_info[f"{x}extent"][f"{x}EX_Extent"][f"{x}description"][char_key]) = d["place"]
...
def overwrite_citation_section(metadata: dict, dataset_info: dict) -> dict:
    ...
    ## Citation Dates
    (citation_info[f"{x}date"][0][f"{x}CI_Date"][f"{x}date"][dt_key]) = d[
        "creation_date"
    ]

    (citation_info[f"{x}date"][1][f"{x}CI_Date"][f"{x}date"][dt_key]) = d[
        "revision_date"
    ]

    ## Citation Contacts
    (
        citation_info[f"{x}citedResponsibleParty"][0][f"{x}CI_ResponsibleParty"][
            f"{x}organisationName"
        ][char_key]
    ) = d["contact_organization"]

    (
        citation_info[f"{x}citedResponsibleParty"][1][f"{x}CI_ResponsibleParty"][
            f"{x}individualName"
        ][char_key]
    ) = d["contact_person"]
...
def overwrite_resource_section(metadata: dict, dataset_info: dict) -> dict:
    ...
    (
        ...
        ][f"{x}MD_MaintenanceFrequencyCode"][code_key]
    ) = d["frequency"]

    (
        maint_info[f"{x}MD_MaintenanceInformation"][
            f"{x}maintenanceAndUpdateFrequency"
        ][f"{x}MD_MaintenanceFrequencyCode"][txt_key]
    ) = d["frequency"]

    ## Constraints
    (constraint_info[0][f"{x}MD_Constraints"][f"{x}useLimitation"][char_key]) = d[
        "public_access"
    ]

    (constraint_info[1][f"{x}MD_LegalConstraints"][f"{x}useLimitation"][char_key]) = d[
        "boilerplate_desc"
    ]

    (constraint_info[2][f"{x}MD_LegalConstraints"][f"{x}useLimitation"][char_key]) = d[
        "boilerplate_license"
    ]
...
def overwrite_data_quality_section(metadata: dict, dataset_info: dict) -> dict:
    ...
    (
        qual_info[f"{x}report"][f"{x}DQ_RelativeInternalPositionalAccuracy"][
            f"{x}measureDescription"
        ][char_key]
    ) = d["horiz_accuracy"]

    (
        qual_info[f"{x}lineage"][f"{x}LI_Lineage"][f"{x}processStep"][
            f"{x}LI_ProcessStep"
        ][f"{x}description"][char_key]
    ) = d["methodology"]
...
def overwrite_distribution_section(metadata: dict, dataset_info: dict) -> dict:
    ...
    dist_info = metadata[f"{x}distributionInfo"][f"{x}MD_Distribution"][
        f"{x}transferOptions"
    ][f"{x}MD_DigitalTransferOptions"][f"{x}onLine"][f"{x}CI_OnlineResource"]
...
if __name__ == "__main__":
    ...
    assert str(Path.cwd()).endswith("open_data"), (
        "this script must be run from open_data directory!"
    )
