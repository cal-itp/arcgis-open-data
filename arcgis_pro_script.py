...
def export_fgdc_metadata(one_feature_class):
    ...
    available.
    """
    this_feature_class = feature_class_in_gdb_path(staging_location, one_feature_class)
    ...
    meta_output = os.path.join(working_dir, f"{one_feature_class}_fgdc.xml")

    TRANSLATOR = "FGDC_CSDGM"

    source_metadata.exportMetadata(
        outputPath=meta_output, metadata_export_option=TRANSLATOR
    )
...
def write_feature_class_to_open_data(
    ...
    """
    staging_feature_class = feature_class_in_gdb_path(staging_gdb, one_feature_class)
    out_feature_class = feature_class_in_gdb_path(output_gdb, one_feature_class)
    ...
    arcpy.conversion.FeatureClassToFeatureClass(
        staging_feature_class, output_gdb, one_feature_class
    )
...
for f in in_features:
    ...
    arcpy.FeatureClassToFeatureClass_conversion(
        in_features=out_feature_class, out_path=ENTERPRISE_DATABASE, out_name=f
    )
