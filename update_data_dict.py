...
def new_columns_for_data_dict(
    ...
    for t in catalog:
        ...
        gdf = (
            gcsgp.read_parquet(FILE)
            .pipe(open_data_utils.standardize_column_names)
            .pipe(open_data_utils.remove_internal_keys)
        )
        ...
        new_cols_dict[t] = [
            c for c in col_list if c not in exclude_me and c not in cols_defined
        ]
...
if __name__ == "__main__":
    ...
    cols_to_add = {k: v for k, v in cols_to_add.items() if len(v) > 0}
